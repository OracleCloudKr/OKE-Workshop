...
1. 배포에 대한 정보를 봅니다.
    ...
    Name:                   hello-world
    Namespace:              default
    CreationTimestamp:      Mon, 10 Sep 2018 14:33:10 +0900
    ...
    OldReplicaSets:  <none>
    NewReplicaSet:   hello-world-5b446dd74b (5/5 replicas created)
    Events:
    Type    Reason             Age   From                   Message
    ----    ------             ----  ----                   -------
    Normal  ScalingReplicaSet  1m    deployment-controller  Scaled up replica set hello-world-5b446dd74b to 5
    ~~~
...
1. ReplicaSet 객체에 대한 정보를 봅니다.
    ...
    hello-world-5b446dd74b   5         5         5         2m
    ~~~
    ~~~
    ...
    Type    Reason            Age   From                   Message
    ----    ------            ----  ----                   -------
    Normal  SuccessfulCreate  2m    replicaset-controller  Created pod: hello-world-5b446dd74b-6n2v6
    Normal  SuccessfulCreate  2m    replicaset-controller  Created pod: hello-world-5b446dd74b-lhndn
    Normal  SuccessfulCreate  2m    replicaset-controller  Created pod: hello-world-5b446dd74b-rwvxm
    Normal  SuccessfulCreate  2m    replicaset-controller  Created pod: hello-world-5b446dd74b-sqhns
    Normal  SuccessfulCreate  2m    replicaset-controller  Created pod: hello-world-5b446dd74b-8tx6x
    ~~~
...
1. 서비스에 대한 정보를 봅니다.
    ...
    my-service   LoadBalancer   10.96.241.141   <pending>     8080:30709/TCP   29s
    ~~~
    ...
    NAME         TYPE           CLUSTER-IP      EXTERNAL-IP     PORT(S)          AGE
    my-service   LoadBalancer   10.96.241.141   129.213.68.63   8080:30709/TCP   9m
    ~~~
...
1. 서비스에 대한 자세한 정보를 봅니다.
    ...
    Type:                     LoadBalancer
    IP:                       10.96.241.141
    LoadBalancer Ingress:     129.213.68.63
    Port:                     <unset>  8080/TCP
    TargetPort:               8080/TCP
    NodePort:                 <unset>  30709/TCP
    Endpoints:                10.244.0.9:8080,10.244.1.4:8080,10.244.1.5:8080 + 2 more...
    ...
    ~~~
    서비스에 노출 된 외부 IP 주소 (LoadBalancer Ingress)를 적어 두십시오. 이 예에서 외부 IP 주소는 129.213.68.63입니다. 또한 Port 및 NodePort의 값을 기록하십시오. 이 예에서 Port는 8080이고 NodePort는 30709입니다.

1. 이전 출력에서는 서비스에 여러 끝점 (10.244.0.9:8080,10.244.1.4:8080,10.244.1.5:8080 + 2)이 있음을 알 수 있습니다. 이 주소는 Hello World 응용 프로그램을 실행중인 포드의 내부 주소입니다. 이 주소가 pod 주소인지 확인하려면 다음 명령을 입력하십시오.

    ~~~
    $ kubectl get pods --output=wide

    NAME                           READY     STATUS    RESTARTS   AGE       IP           NODE
    hello-world-5b446dd74b-6n2v6   1/1       Running   0          8m        10.244.2.5   129.213.96.143
    hello-world-5b446dd74b-8tx6x   1/1       Running   0          8m        10.244.2.6   129.213.96.143
    hello-world-5b446dd74b-lhndn   1/1       Running   0          8m        10.244.1.4   129.213.62.253
    hello-world-5b446dd74b-rwvxm   1/1       Running   0          8m        10.244.0.9   129.213.88.191
    hello-world-5b446dd74b-sqhns   1/1       Running   0          8m        10.244.1.5   129.213.62.253
    ~~~

1. 외부 IP 주소 (LoadBalancer Ingress)를 사용하여 Hello World 응용 프로그램에 액세스하십시오.
    ~~~
    $ curl http://<external-ip>:<port>
    $ curl http://129.213.68.63:8080

    Hello Kubernetes!%
    ~~~
    여기서 \<external-ip>는 서비스의 외부 IP 주소 (LoadBalancer Ingress)이고 \<port>는 서비스 설명의 Port 값입니다.

    성공적인 요청에 대한 응답은 `Hello Kubernetes!%`입니다.
